...
Windows Desktop, the loopback gateway, the single DeepSeek provider, the community-compatible Claude Code runtime, and credential-source hygiene pass. Codex has static Responses compatibility only, not CCR runtime. Multi-provider architecture is documented, while switching runtime remains untested. V0.4 is published as the Advanced / Experimental `v0.4.0-rc.1` release candidate, not a stable release.

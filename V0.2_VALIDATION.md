...
> Status: DeepSeek Runtime Gate 1 PASS; Bailian Runtime Gate 2 PASS; Open WebUI Cloud BLOCKED
...
## Open WebUI preflight finding

The maintainer's existing legacy `open-webui` container currently publishes port 3000 on all IPv4/IPv6 interfaces (`0.0.0.0` / `::`). No cloud Provider credential was added. Runtime integration is blocked until the instance is migrated to loopback-only binding or an isolated loopback-only test instance is used.

...
| Agent Lab 基线 | PASS | Python 标准库，3 tests |
...
- 结果：3 tests PASS；作用域检查 PASS；只修改 `src/` 与 `tests/`
...
Static bilingual docs, links, the three-test Agent Lab, and isolated workspace preparation pass. Codex completed the scoped median task from a normal Windows PowerShell session, passed all tests and scope checks, and did not use networking, dependency installation, commits, pushes, Open WebUI, or Docker. Codex runtime is independent of Open WebUI. The legacy port-3000 instance remains separate security debt and is explicitly excluded from V0.3/V0.4 runtime. Claude Code remains documentation-only without an official Anthropic credential; its DeepSeek route is a community-compatible experiment, not an Anthropic-supported model route. Evidence excludes identity, credentials, full user paths, full prompts, and account pages.
